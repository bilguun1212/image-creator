...
export const FoodGeneration = () => {
  ...
  const generateImageAndExtract = async () => {
    ...
    try {
      ...
      const extractedIngredeients = await ingredients.json();
      console.log(extractedIngredeients);

      if (!response.ok) {
        throw new Error(generateImage.error || "error");
      }

      if (!generateImage.image) {
        throw new Error(generateImage.image);
      }

      if (extractedIngredeients) {
        setExtractedInfo(
          extractedIngredeients.split(",").map((item: string) => item.trim()),
        );
      }
    } catch (error) {
